...
export const zh: Translations = {
  ...
  settings: {
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让萌学伴跟随系统设置。',
  ...
  commandCenter: {
    ...
    gatewayNotConfigured: '消息渠道未启用',
    hermesActiveSessions: (version, count) => `萌学伴 ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    gatewayRestartFailed: '网关重启失败。',
    updateHermes: '更新萌学伴',
    updateManagedExternally: '由管理员更新容器',
    actionRunning: '运行中',
    actionDone: '完成',
    actionFailed: '失败',
  ...
  updates: {
    ...
    applyingBody: '萌学伴更新程序会在独立窗口中完成更新，并在结束后自动重新打开萌学伴。更新期间请不要手动重启。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Hermes 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后萌学伴会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    localStartUnavailable: '无法启动本地安装。请重启萌学伴后重试。',
    remoteSetupTitle: '连接到现有 Hermes',
    remoteSetupDesc: '输入网关 URL。萌学伴会检测需要令牌还是浏览器登录。',
  ...
  onboarding: {
    headerTitle: '开始设置萌学伴',
    headerDesc: '连接模型服务即可开始使用萌学伴，大多数选项只需一次点击。',
    preparingInstall: '萌学伴正在完成首次安装，通常不到一分钟。',
    starting: '萌学伴正在启动…',
  ...
  shell: {
    ...
    statusbar: {
      ...
      desktopVersion: version => `萌学伴 v${version}`,
  ...
  education: {
    ...
    home: {
      eyebrow: '教研工作台',
      title: '今天想完成什么？',
      description: '从目标开始，萌学伴会为你准备角色、资料来源、执行步骤和最终产物。',
      intentPlaceholder: '例如：整理小学五年级数学沪教版教材课程树',
      intentAction: '准备任务',
      scenariosTitle: '常用教研场景',
      scenariosDescription: '已经定义必要范围和预期产物，选择后再确认细节，不会直接执行。',
      continueTitle: '继续处理',
      continueEmpty: '暂无需要处理的任务',
      continueEmptyDescription: '等待选择或正在执行的任务会显示在这里。',
      sourceTitle: '数据来源状态',
      sourceSummary: (available, total) => `${total} 个来源中 ${available} 个可用`
    },
    scenario: {
      courseTreeName: '教材课程树',
    ...
    tasks: {
      title: '任务',
      description: '处理等待确认的问题、查看执行进度并打开已经完成的产物。',
      createTitle: '准备任务',
      createDescription: '所选场景会先收集明确目的和必要范围，确认后才进入执行。',
      needsAttention: '需要我处理',
      running: '进行中',
      completed: '已完成',
      partial: '部分完成',
      empty: '还没有任务',
      emptyDescription: '从首页选择一个教研场景，准备第一个任务。',
      purpose: '这次要完成什么',
      purposePlaceholder: '说明使用对象、预期用途和希望得到的结果',
      fieldPlaceholder: field => `请选择或填写${field}`,
      continueToConfirm: '检查任务内容',
      confirmTitle: '执行前确认',
      confirmDescription: '确认范围和产物后才会创建任务；发现多个教材版本时仍会再次请你选择。',
      editInformation: '返回修改',
      confirmCreate: '确认创建任务',
      created: '任务已创建，等待执行连接。',
      requiredHint: '必填',
      startExecution: '开始执行',
      startingExecution: '正在连接执行端…',
      openConversation: '打开执行会话',
      executionFailed: '执行启动失败，请检查 Gateway 连接后重试。',
      answerFailed: '提交选择失败，请重试或打开执行会话处理。',
      openArtifacts: '查看产物'
    },
    sources: {
      title: '数据来源',
      description: '查看每个来源可以做什么、支持什么范围以及当前是否可用。',
      capability: '可用能力',
      capabilityCatalogue: '目录采集',
      capabilityRead: '内容读取',
      capabilityRefresh: '定期更新',
      capabilitySearch: '资料搜索',
      scope: '支持范围',
      state: '当前状态',
      available: '可用',
      notConfigured: '未配置',
      empty: '还没有数据来源',
      emptyDescription: '配置后的教材来源、知识库、联网搜索和文件会显示在这里。'
    },
    templates: {
      title: '角色与场景',
      description: '查看内置工作方式，或创建自己的副本进行调整。',
      safetyTitle: '内置模板始终保留',
      safetyDescription: '你可以直接使用内置模板。需要调整时先创建个人副本，修改不会影响其他用户或官方更新。',
      rolesTitle: '角色',
      rolesDescription: '角色决定助手如何理解任务、使用能力和组织结果。',
      scenesTitle: '场景',
      scenesDescription: '场景定义需要确认的信息、使用的角色和最终产物。',
      builtin: '内置',
      personal: '我的',
      createCopy: '创建我的副本',
      edit: '编辑',
      ...
      instruction: '工作要求',
      instructionHint: '说明这个角色应该如何判断、确认和输出。',
      assignedRole: '使用角色',
      inputs: count => `${count} 项必要信息`,
      outputs: count => `${count} 个产物`,
      saved: '已保存',
      saveFailed: '保存失败，请重试。'
    }
  },
