...
/// Relaunch an explicit branded desktop target supplied by the running app.
/// This keeps white-label shells on their own executable after the shared
/// Hermes runtime updates instead of opening the upstream release artifact.
pub(crate) fn spawn_desktop_target(target: &std::path::Path) -> std::io::Result<()> {
    #[cfg(target_os = "macos")]
    if target.extension().and_then(|e| e.to_str()) == Some("app") {
        return open_macos_app_detached(target);
    }

    let mut cmd = std::process::Command::new(target);
    cmd.current_dir(target.parent().unwrap_or_else(|| std::path::Path::new(".")));
    #[cfg(target_os = "windows")]
    {
        use std::os::windows::process::CommandExt;
        cmd.creation_flags(0x0000_0008);
