...
const ARTIFACT_PREFIX = `MengXueBan-${PACKAGE_JSON.version}`
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `${ARTIFACT_PREFIX}-mac-${ARCH}.dmg`)
  }

  const prefix = ARTIFACT_PREFIX
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `${ARTIFACT_PREFIX}-mac-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder artifactName template is 'MengXueBan-${version}-${os}-${arch}.${ext}'
